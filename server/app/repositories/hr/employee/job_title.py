...
class JobTitleRepository:
    ...
    async def get_job_titles(self, skip: int, limit: int) -> tuple[list[JobTitle], int]:
        data = self.db.query(JobTitle).offset(skip).limit(limit).all()
        total_rows = self.db.execute(select(func.count(JobTitle.id))).scalar()
